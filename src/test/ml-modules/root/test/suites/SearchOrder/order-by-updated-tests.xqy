...
(: suite-setup leaves last-modified in the opposite order to decision date:
   newest touched first, oldest touched last. Ascending updated should therefore
   return newest → middle → oldest, not the date order. :)
...
return (
  ...
  test:assert-equal("/order/oldest.xml", $descending-uris[1]),
